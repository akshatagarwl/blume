...
Set `ai.api` to `false` to publish none of it. A `public/openapi.json` you ship yourself takes over that route (the JSON endpoints stay), and a custom page owning a rest route under `/api/` keeps its own not-found handling:
