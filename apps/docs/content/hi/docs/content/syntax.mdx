...
एंकर आईडी शीर्षक के पाठ से उत्पन्न होती हैं, इसलिए शब्द बदलने पर शीर्षक को नई एंकर मिल जाती है। इसके बजाय एंकर को स्थिर करने के लिए `[#custom-id]` जोड़ें — यह मार्कर कभी रेंडर नहीं होता, और शीर्षक चाहे जैसा भी पढ़ा जाए, लिंक काम करते रहते हैं। स्थिर की गई एंकर [अनूदित लोकेल](/docs/content/i18n) में भी एक जैसी रहती हैं, जबकि स्वतः उत्पन्न आईडी हर भाषा में अलग-अलग होतीं।

```md
## Getting started [#setup]
```

इसे `/page#setup` के रूप में लिंक करें। यह सिंटैक्स Fumadocs से मेल खाता है, इसलिए माइग्रेट किया गया कंटेंट ज्यों का त्यों काम करता है।

Pandoc, kramdown, और Markdown-आधारित विनिर्देश टूलचेन द्वारा उपयोग किया जाने वाला `{#custom-id}` रूप `.md` फ़ाइलों में समकक्ष के रूप में स्वीकार किया जाता है। `.mdx` में, अकेला `{…}` एक JSX एक्सप्रेशन होता है और पेज कंपाइल होने में विफल हो जाता है — `blume check` इस मार्कर को `BLUME_MDX_CURLY_ANCHOR` के रूप में रिपोर्ट करता है — इसलिए वहाँ `[#custom-id]` लिखें, या ब्रेसेस को एस्केप करें। एस्केप किया गया रूप दोनों फ़ॉर्मैट में एक ही एंकर स्थिर करता है, जिससे यह उस पार्शियल के लिए सही वर्तनी बन जाता है जिसे `.mdx` पेज [इनक्लूड](/docs/content/includes) करते हैं:

```md
## Getting started \{#setup\}
```

फ़्रैगमेंट लिंक `id` वाले किसी रॉ HTML एलिमेंट (`<a id="setup"></a>`) को भी लक्षित कर सकते हैं; `blume validate` उन्हें शीर्षक एंकर के साथ-साथ स्वीकार करता है।

### विषय-सूची मार्कर

दो और अनुगामी मार्कर यह नियंत्रित करते हैं कि कोई शीर्षक विषय-सूची में कैसे दिखे। `[!toc]` शीर्षक को पेज पर रखता है पर TOC से बाहर; `[toc]` इसका उलटा करता है — शीर्षक केवल TOC में दिखता है, एक अदृश्य एंकर लक्ष्य के रूप में, जो गद्य के बजाय कंपोनेंट्स से बने अनुभागों को लेबल करने के लिए उपयोगी है। मार्कर किसी भी क्रम में जोड़े जा सकते हैं। एक अपवाद, जो CommonMark से विरासत में मिला है: ऐसा अनुगामी ब्रैकेट जिसके लेबल की कोई लिंक-रेफ़रेंस परिभाषा पेज पर कहीं भी मौजूद हो (`[toc]: /url`), वह मार्कर नहीं बल्कि एक शॉर्टकट रेफ़रेंस लिंक होता है और शीर्षक के पाठ में बना रहता है।
...
मार्कर हमेशा पार्स होते हैं — ऐसा शीर्षक जो शाब्दिक रूप से मार्कर-जैसे पाठ पर समाप्त होता है, चिह्नित माना जाएगा। बैकस्लैश-एस्केपिंग से मदद नहीं मिलती (मार्कर पार्स चलने से पहले Markdown `\[` को `[` में हल कर देता है); किसी शीर्षक के अंत में शाब्दिक मार्कर पाठ दिखाने के लिए, उसे इनलाइन कोड में लपेटें: `` ## Using `[toc]` ``।
...
### TypeScript और JavaScript टैब

किसी `ts` या `tsx` ब्लॉक को `ts2js` से चिह्नित करें ताकि वह टैब जोड़ी के रूप में रेंडर हो: आपका TypeScript और उसके साथ स्वतः उत्पन्न JavaScript संस्करण, ताकि आप एक ही स्निपेट संभालें और पाठक अपनी बोली चुन लें। यह रूपांतरण टाइप सिंटैक्स और केवल-टाइप इम्पोर्ट हटा देता है, जबकि आपकी फ़ॉर्मेटिंग, टिप्पणियाँ, और JSX ठीक वैसे ही रखता है जैसे लिखे गए हैं — और टैब सिंक होते हैं, इसलिए एक बार JavaScript चुनने से पेज की हर जोड़ी बदल जाती है। डायग्राम और गणित की तरह, यह भी केवल-MDX फ़ीचर है — किसी `.md` फ़ाइल में यह ब्लॉक एक सादे TypeScript फ़ेंस के रूप में रेंडर होता है।
...
अन्य फ़ेंस मेटा भी साथ काम करता है: `title="..."` दोनों टैब पर दिखता है, जबकि `{1,4-5}` लाइन रेंज केवल TypeScript टैब पर लागू होती हैं (टाइप हटने के बाद लाइन नंबर खिसक जाते हैं)। एकमात्र अपवाद `twoslash` है — होवर टाइप उत्पन्न कोड तक नहीं ले जाए जा सकते, इसलिए दोनों से चिह्नित फ़ेंस एक सादा Twoslash ब्लॉक ही रहता है।
...
एक `package-install` ब्लॉक एकल इंस्टॉल कमांड को npm, pnpm, yarn, bun, nub, और aube के लिए टैब वाले स्निपेट में बदल देता है — ताकि पाठक वही कॉपी करें जो उनके सेटअप से मेल खाता है। डायग्राम और गणित की तरह, यह भी केवल-MDX फ़ीचर है — किसी `.md` फ़ाइल में यह ब्लॉक एक सादे कोड फ़ेंस के रूप में रेंडर होता है।
...
:::note
गणित केवल ब्लॉक-स्तर पर है और स्वतः चालू है — `$$…$$` लिखें और यह रेंडर हो जाता है; न लिखें तो KaTeX की स्टाइलशीट कभी शिप नहीं होती। इनलाइन `$…$` गणित नहीं है: अकेला `$` (मुद्रा, शेल वेरिएबल, कोड) हमेशा शाब्दिक पाठ ही रहता है, इसलिए न कोई डीलिमिटर एस्केप करना है और न कोई सेटिंग टॉगल करनी है। गणित एक केवल-MDX फ़ीचर है। रेंडर किए गए मार्कअप के भीतर के क्लास नाम (`.katex-html`, `.katex-base`, …) KaTeX के अपने आंतरिक विवरण हैं, कोई ऐसा स्टाइलिंग अनुबंध नहीं जिसे Blume बनाए रखता हो — KaTeX के अपग्रेड होने पर वे बदल सकते हैं, इसलिए कस्टम स्टाइल के लिए `.katex-display` रैपर को लक्षित करें।
:::
