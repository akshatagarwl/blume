...
Blume में कुछ AI सुविधाएँ हैं: बाहरी टूल्स के लिए मशीन-पठनीय डॉक्स (`llms.txt` और एक OpenAPI विवरण के साथ एक [JSON API](#json-api), दोनों डिफ़ॉल्ट रूप से चालू), एक इन-पेज **Ask AI** सहायक, और कोडिंग एजेंट्स के लिए एक होस्टेड **MCP सर्वर**। Ask AI और MCP ऑप्ट-इन हैं, और जब तक आप कोई सुविधा चालू नहीं करते, स्टैटिक डॉक्स पूरी तरह स्टैटिक बने रहते हैं।
...
`llms.txt` का अंत दो जनरेट किए गए अनुभागों से होता है जिन्हें किसी कॉन्फ़िगरेशन की ज़रूरत नहीं। **Agent skills** [`ai.skills`](#skills-discovery) के माध्यम से प्रकाशित प्रत्येक स्किल को उसके विवरण के साथ सूचीबद्ध करता है (जहाँ कोई स्किल बताती है कि उसका उपयोग कब करना है)। **Agent resources** बिल्ड द्वारा उत्सर्जित हर मशीन-पठनीय आर्टिफ़ैक्ट को लिंक करता है — `llms-full.txt`, प्रति-पेज [कच्चा Markdown](#raw-markdown) प्रतिबिंब, [MCP सर्वर](#mcp-server) और उसका डिस्कवरी दस्तावेज़, स्किल्स इंडेक्स, [API कैटलॉग](#api-catalog), [`agent-readability.json`](#agent-readability), और [साइटमैप](/docs/configuration/seo#sitemap) — प्रत्येक केवल तभी जब वह मौजूद हो, ताकि जो एजेंट `llms.txt` के अलावा कुछ न पढ़े, वह भी पूरी सतह खोज ले।
...
अनुपस्थित पेज भी नेगोशिएट करते हैं। हर बिल्ड `/404.md` पर एक Markdown [404 पेज](/docs/advanced/custom-pages#404-page) उत्सर्जित करता है — नहीं-मिला संदेश, उसके बाद हर शीर्ष-स्तरीय अनुभाग, साइटमैप, और `llms.txt` की ओर पुनर्प्राप्ति लिंक — और Vercel पर, किसी अस्तित्वहीन URL के लिए ऐसा अनुरोध जो Markdown को प्राथमिकता देता है, या ऐसा कोई भी `.md` URL जिसके पीछे कोई पेज न हो, HTML शेल के बजाय वही बॉडी एक वास्तविक `404` स्थिति के साथ प्राप्त करता है।
...
यह प्रॉम्प्ट [UI शब्दकोश](/docs/content/i18n#translated-ui) (`actions.openInChatPrompt`) का हिस्सा है, इसलिए स्थानीयकृत साइटें इसे अपनी भाषा में भेजती हैं, और `i18n.ui` शब्दावली को ओवरराइड कर सकता है — `{url}` प्लेसहोल्डर बनाए रखें, जिसे पेज के कच्चे-Markdown URL से बदल दिया जाता है।
...
हर Blume साइट अपने डॉक्स को एक छोटे केवल-पढ़ने योग्य **JSON API** के रूप में भी सर्व करती है — [MCP सर्वर](#mcp-server) के टूल्स का REST जुड़वाँ, उसी पेज स्नैपशॉट के ऊपर, उन एजेंट्स और फ़ंक्शन-कॉलिंग फ़्रेमवर्क्स के लिए जो MCP के बजाय सादा HTTP बोलते हैं। यह डिफ़ॉल्ट रूप से चालू है और इसे किसी कॉन्फ़िगरेशन की ज़रूरत नहीं:
...
पेज इंडेक्स, प्रति-पेज दस्तावेज़, और नेविगेशन पूर्व-रेंडर किए जाते हैं, इसलिए एक स्टैटिक साइट उन्हें किसी भी होस्ट से फ़ाइलों के रूप में सर्व करती है। खोज एक लाइव एंडपॉइंट है और केवल [सर्वर आउटपुट](/docs/deployment#server-rendering) के अंतर्गत मौजूद रहती है, जहाँ यह वही इंडेक्स चलाती है जो `search_docs` चलाता है। त्रुटियाँ [RFC 9457](https://www.rfc-editor.org/rfc/rfc9457) प्रॉब्लम डिटेल्स (`application/problem+json`) होती हैं, जिनमें एक स्थिर `code`, एक `detail`, और एक `resolution` संकेत होता है जो एजेंट को बताता है कि आगे कहाँ जाना है — कोई अनुपस्थित पेज, कोई खाली खोज क्वेरी, या सर्वर आउटपुट पर कोई भी ऐसा `/api/…` URL जिसका उत्तर कोई एंडपॉइंट न देता हो:
...
`/openapi.json` पर मौजूद **OpenAPI दस्तावेज़** आपके कॉन्फ़िग से प्रति बिल्ड जनरेट होता है, इसलिए यह केवल उसी का वर्णन करता है जो डिप्लॉय की गई साइट सर्व करती है: हर JSON एंडपॉइंट एक अद्वितीय `operationId`, टाइप किए गए पैरामीटर्स, और प्रतिक्रिया स्कीमा के साथ, साथ ही उसके बगल की टेक्स्ट सतहें — [`.md` प्रतिबिंब](#raw-markdown), [`llms.txt`](#llmstxt) और `llms-full.txt`, [`agent-readability.json`](#agent-readability) — और [MCP एंडपॉइंट](#mcp-server) जब वह सक्षम हो। जो फ़्रेमवर्क किसी OpenAPI विवरण से टूल्स बनाते हैं, उन्हें वही पहुँच मिलती है जो एक MCP क्लाइंट के पास होती है। यह दस्तावेज़ [API कैटलॉग](#api-catalog), [रीडेबिलिटी मैनिफ़ेस्ट](#agent-readability), होमपेज के `Link` हेडर में `rel="service-desc"` के रूप में, और `llms.txt` से लिंक किया जाता है।

इसमें से कुछ भी आपके अपने [API संदर्भ](/docs/advanced/api-reference) को नहीं छूता: कोई दस्तावेज़ीकृत स्पेक पेजों में रेंडर किया जाता है, कभी `/openapi.json` पर सर्व नहीं होता, और कैटलॉग दोनों को सूचीबद्ध करता है। आपके द्वारा स्वयं भेजी गई `public/openapi.json` उस रूट को अपने अधिकार में ले लेती है (JSON एंडपॉइंट बने रहते हैं)। `/api/…` कैच-ऑल तब पीछे हट जाता है जब कोई डॉक्स अनुभाग `/api` नेमस्पेस से सर्व होता है (`content/api/overview.md`) या कोई कस्टम पेज `/api/` के अंतर्गत किसी शेष रूट का स्वामी हो, ताकि वे पेज जीतते रहें। इसमें से कुछ भी प्रकाशित न करने के लिए `ai.api` को `false` सेट करें:
...
मैनिफ़ेस्ट केवल वही सूचीबद्ध करता है जो आपने सक्षम किया है — [कच्चा Markdown](#raw-markdown) प्रतिबिंब पैटर्न, [JSON API](#json-api) और उसका OpenAPI विवरण, [`llms.txt`](#llmstxt) और `llms-full.txt`, [MCP सर्वर](#mcp-server) और उसका डिस्कवरी दस्तावेज़, [Ask AI](#ask-ai) एंडपॉइंट, [साइटमैप](/docs/configuration/seo#sitemap), और [RSS फ़ीड्स](/docs/configuration/seo#rss-feeds) — साथ ही आपकी साइट का नाम, विवरण, स्रोत रिपॉज़िटरी, और [content-signal](/docs/configuration/seo#content-signals) उपयोग नीति। [`deployment.site`](/docs/deployment) सेट होने पर URL निरपेक्ष होते हैं और अन्यथा रूट-सापेक्ष:
...
प्रत्येक प्रविष्टि केवल तभी दिखती है जब उसकी सुविधा चालू हो। `alternate` लिंक होमपेज के Markdown प्रतिबिंब की ओर इंगित करता है — जब होम रूट एक कंटेंट पेज हो तो पेज का अपना [कच्चा Markdown](#raw-markdown), या जब वह लैंडिंग पेज हो तो संश्लेषित `llms.txt` फ़ॉलबैक। `service-desc` लिंक ([RFC 8631](https://www.rfc-editor.org/rfc/rfc8631)) [JSON API](#json-api) के OpenAPI विवरण की ओर इंगित करता है, और `api-catalog` [जनरेट किए गए API कैटलॉग](#api-catalog) की ओर। हेडर हर उस सतह पर जाता है जिसे Blume नियंत्रित करता है: dev सर्वर (`curl -I localhost:4321` से जाँचें), उत्सर्जित `_headers` फ़ाइल के माध्यम से स्टैटिक बिल्ड (Netlify और Cloudflare), और डि��्लॉय के रूटिंग नियमों के माध्यम से Vercel सर्वर बिल्ड।
...
बिना किसी API संदर्भ, बिना MCP सर्वर, और [JSON API](#json-api) बंद किए हुए साइट कोई कैटलॉग उत्सर्जित नहीं करती — उसमें कुछ होता ही नहीं। हर जगह की तरह, आपके द्वारा स्वयं भेजी गई `public/.well-known/api-catalog` फ़ाइल जनरेट की गई फ़ाइल पर भारी पड़ती है।
