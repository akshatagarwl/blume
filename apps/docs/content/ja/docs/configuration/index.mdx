...
メタデータ、Open Graph 画像、RSS フィード、JSON-LD、サイトマップ、`robots.txt` は `seo` の下にまとまっています。`llms.txt`、生の Markdown、JSON API、MCP サーバーは `ai` の下です。どちらも [発見可能性](/docs/discoverability) セクションでページごとに解説しており、そこでは検索エンジンと AI エージェントを、同じ機械可読レイヤーに対する 2 つのオーディエンスとして扱っています。
...
| `ai` | `llms.txt`、Markdown ミラー、JSON API、コーディングエージェント向けのホスト型 MCP サーバー | [SEO と AEO](/docs/discoverability) |
| `ai.ask` | ページ内の Ask AI アシスタント | [Ask AI](/docs/configuration/ask-ai) |
