...
- **`subsets`** — 読み込む文字サブセット。プロバイダーの名前で指定します（`latin`、`latin-ext`、`vietnamese`、`cyrillic`、`greek` など）。デフォルトは `latin` に加えて、設定したロケールが必要とするもの — 詳しくは以下を参照してください。
- **`fallback`** — フォントの読み込み中や欠けているグリフに使われるシステムスタック: `sans`、`serif`、`mono`。mono の役割では `mono`、それ以外では `sans` がデフォルトです。

#### サブセットとロケール

Google、Bunny、Fontsource は各ファミリーを文字体系ごとのサブセットに分割しており、`@font-face` が生成されるのは読み込んだサブセットだけです。Blume はそのリストを [`i18n.locales`](/docs/content/i18n) から導き出します。ベトナム語、ポーランド語、ロシア語、ギリシャ語のロケールを持つサイトでは `latin` に加えて `vietnamese`、`latin-ext`、`cyrillic`、`greek` が読み込まれるため、ダイアクリティカルマークや非ラテン文字がシステムフォールバックではなく選んだフォントで表示されます。`i18n` ブロックがないサイトや、Latin-1 の言語だけのサイトでは `latin` のみが読み込まれます。ブラウザーはそのサブセットの文字をページが使っている場合にのみダウンロードし、プリロードも同じリストに従います。

導き出されたリストを上書きしたい場合は、ファミリーに `subsets` を設定します。たとえば、単一ロケールのサイトでコンテンツがそのロケールからは導かれない文字体系を必要とする場合です:
...
`inter` のような厳選スラッグもロケールから導かれたリストに従います。それらのファミリーでサブセットを固定したい場合もオブジェクト形式を使ってください。
...
:::note
`theme.fonts` を明示的に設定すると、display と body のフォントは生成される [Open Graph カード](/docs/discoverability/open-graph#card-fonts)にも自動的に適用されるため、共有されたリンクがサイトと揃います。Google 以外のプロバイダーのファミリーはそこではスキップされます（カードのレンダラーは Google Fonts からしか取得できません）。ローカルファイルはどこでも動作します。
:::
...
`theme.css` はサイトの Tailwind エントリにインライン展開されるため、その中で Tailwind のディレクティブも使えます。モノレポで知っておきたいのが `@source` です。Blume はユーティリティクラスを求めてプロジェクトをスキャンしますが、兄弟のワークスペースパッケージからコンポーネントを import しているページでは、そのパッケージもスキャンする必要があります。`theme.css` からの相対パスで指定すれば — これは Tailwind の標準的なルールです — Blume がそのパスを生成されるスタイルシートに引き継ぎます:
