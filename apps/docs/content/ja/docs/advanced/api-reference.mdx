...
上で説明した内容はすべてそのまま引き継がれます。[`playground`](#イベント向けの-try-it) も含めて: `route`、`label`/`route` 付きの `sources`、`expandSchemas`、[ソースごとのインデックス](#ソースごとのインデックス)のフラグ（`seoDescriptionSuffix` も含みます — 生成される一文はエンドポイントではなくチャネルとアクションを挙げます）、そして操作のサマリーとタグによる検索インデックスです。
