...
Die Seite hat außerdem ein Markdown-Gegenstück unter `/404.md` mit denselben Rückkehr-Links (absolute URLs, sobald [`deployment.site`](/docs/deployment) gesetzt ist). Bei einem [Vercel-Server-Build](/docs/deployment#server-rendering) erhält eine Anfrage nach einer fehlenden Seite, die [`Accept: text/markdown`](/docs/configuration/ai#content-negotiation) sendet oder eine `.md`-URL anfordert, hinter der keine Seite steht, diesen Markdown-Textkörper mit dem Status `404` statt der HTML-Hülle — sodass ein Agent nie eine Seite voller Chrome parsen muss, um zu erfahren, wohin es weitergeht.
