...
Blume erzeugt automatisch einen RSS-Feed für jeden Inhaltstyp, der in [`rss.types`](/docs/discoverability/rss) aufgeführt ist — standardmäßig `blog` und `changelog` — sofern er mindestens eine Seite enthält. Feeds werden unter `/<type>/rss.xml` ausgeliefert:
