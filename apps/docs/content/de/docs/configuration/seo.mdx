...
Standardmäßig leitet sich jede Karte aus deinen Inhalten und deinem Theme ab – der **Seitentitel** als Überschrift, die **Seitenbeschreibung** als Untertitel (derselbe Text wie ihre `og:description`, sodass `seo.description` Vorrang vor `description` hat), der **Titel deiner Site** als Kopfzeile und der **Akzent** deines Themes für die Marke. Bilder werden unter `/og/<slug>.png` ausgeliefert, spiegeln also jede Route, und werden selbst im Server-Modus als statische Dateien vorgerendert:
...
Über die Überschrift hinaus trägt die Karte drei optionale Ebenen: die **Markenkennzeichnung** oben links (dein Logo oder eine Akzentkachel mit dem Anfangsbuchstaben des Site-Titels), den **Untertitel** unter der Überschrift (die `description` der Seite oder die `description` deiner Site bei Seiten ohne eigene) und eine **Fußzeile** mit deinem Repo-Slug (aus `github`) und der URL der Site – dem Host der Deployment-Site plus [`deployment.base`](/docs/deployment#subpath-deploys), sodass eine GitHub-Pages-Projektseite als `user.github.io/repo` erscheint. Überschreibe jede davon mit einer eigenen Zeichenkette oder blende eine mit `false` aus:
...
Der WebSite-Knoten sagt, dass eine Site existiert; er sagt nicht, _was_ sie ist oder _wer_ sie betreibt – und genau dafür lesen KI-Agents JSON-LD, bevor sie dich empfehlen oder zitieren. Zwei optionale Blöcke füllen das aus, beide benötigen `deployment.site` (die Knoten tragen absolute Bezeichner):
...
`organization` fügt jeder Seite einen **Organization**-Knoten hinzu – der WebSite-Knoten und die Artikel-Knoten führen ihn als `publisher` – mit E-Mail und Telefonnummer als `ContactPoint` (`contactType` ist standardmäßig `"customer support"`) und der Adresse als `PostalAddress`, den beiden Feldern, nach denen Prüfungen zur Unternehmensverifizierung suchen. `name` und `url` übernehmen standardmäßig die Werte der Site; ein wurzelrelatives `logo` wird wie jede Seiten-URL absolut gemacht.

`software` fügt der Startseite einen **SoftwareApplication**-Knoten hinzu: Name und Beschreibung des Produkts (ersatzweise die der Site), `applicationCategory` (Standard `"DeveloperApplication"`), Betriebssystem, Lizenz, ein `Offer`, wenn `price` gesetzt ist, und die Registry- oder Repository-URLs in `sameAs`. Übergib `software: true`, um alle Standardwerte zu übernehmen. Die Organisation wird, sofern konfiguriert, als dessen `publisher` geführt.
