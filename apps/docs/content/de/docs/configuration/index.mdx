...
Open-Graph-Bilder, RSS-Feeds und strukturierte JSON-LD-Daten, gruppiert unter `seo`. Metadaten, Frontmatter-Überschreibungen und die vollständige Referenz findest du in der [SEO-Anleitung](/docs/discoverability).
...
| `ai` | `llms.txt`, Ask AI und der gehostete MCP-Server für Coding-Agents | [KI](/docs/discoverability) |
| `analytics` | Vercel, PostHog und benutzerdefinierte Skripte | [Analytics](/docs/configuration/analytics) |
| `seo` | Metadaten, OG-Bilder, Feeds, strukturierte Daten | [SEO](/docs/discoverability) |
