...
- `og:image` — a [imagem OG](#open-graph-images) da página. Um card gerado também declara seus `og:image:width`, `og:image:height`, `og:image:type` e `og:image:alt`, para que um rastreador possa dispor o card sem precisar buscá-lo antes; uma `seo.image` fornecida por você não declara nenhum desses, já que seu tamanho e formato são desconhecidos.
...
Por padrão, cada card é derivado do seu conteúdo e tema — o **título da página** como manchete, a **descrição da página** como subtítulo (o mesmo texto da sua `og:description`, então `seo.description` prevalece sobre `description`), o **título do site** como sobretítulo e o **accent** do seu tema para a marca. As imagens são servidas em `/og/<slug>.png`, espelhando cada rota, e são pré-renderizadas como arquivos estáticos mesmo em modo servidor:
...
Além da manchete, o card carrega três camadas opcionais: a **marca** no canto superior esquerdo (seu logotipo, ou um bloco na cor accent com a inicial do título do site), o **subtítulo** abaixo da manchete (a `description` da página, ou a `description` do seu site nas páginas sem uma) e um **rodapé** com o slug do seu repositório (a partir de `github`) e a URL do site — o host do site de deployment mais [`deployment.base`](/docs/deployment#subpath-deploys), de modo que um site de projeto no GitHub Pages exibe `user.github.io/repo`. Sobrescreva qualquer uma delas com um texto seu, ou oculte uma com `false`:
...
O nó WebSite diz que um site existe; ele não diz _o que_ ele é nem _quem_ o mantém — que é justamente o que os agentes de IA leem no JSON-LD antes de recomendar ou citar você. Dois blocos opcionais preenchem essa lacuna, ambos precisando de `deployment.site` (os nós carregam identificadores absolutos):
...
`organization` adiciona um nó **Organization** a cada página — os nós WebSite e de artigo o citam como `publisher` — com o e-mail e o telefone como um `ContactPoint` (`contactType` assume `"customer support"` por padrão) e o endereço como um `PostalAddress`, os dois campos que as verificações de negócio procuram. `name` e `url` assumem os do site por padrão; um `logo` relativo à raiz é convertido em absoluto como qualquer URL de página.

`software` adiciona um nó **SoftwareApplication** à página inicial: o nome e a descrição do produto (assumindo os do site por padrão), `applicationCategory` (padrão `"DeveloperApplication"`), sistema operacional, licença, um `Offer` quando `price` está definido, e as URLs de registro ou repositório em `sameAs`. Passe `software: true` para aceitar todos os padrões. A organização, quando configurada, é citada como seu `publisher`.
...
O Blume escreve um `robots.txt` que permite todos os rastreadores, declara seus [sinais de conteúdo](#content-signals) e adiciona uma linha `Sitemap:` apontando para o sitemap quando há um disponível. Ativado por padrão:
