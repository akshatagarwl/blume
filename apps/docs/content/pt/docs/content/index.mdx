...
O Blume gera automaticamente um feed RSS para cada tipo de conteúdo listado em [`rss.types`](/docs/discoverability/rss) — `blog` e `changelog` por predefinição — desde que tenha pelo menos uma página. Os feeds são servidos em `/<type>/rss.xml`:
