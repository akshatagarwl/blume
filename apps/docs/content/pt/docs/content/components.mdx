...
Agrupe vários blocos de código em um único alternador com abas — uma aba por linguagem ou arquivo. O rótulo da aba é o título de cada bloco (o texto após a linguagem), e o botão de cópia do grupo fica na barra de abas e copia o bloco que está sendo exibido. Adicione `dropdown` para alternar com um menu em vez de uma barra de abas.
...
Em um monorepo, os componentes que seus exemplos importam geralmente ficam em um pacote irmão do workspace. O `@source` do Tailwind varre arquivos, não imports: o Blume varre o seu projeto e o diretório `examples` (para onde quer que `source` aponte, mesmo fora do projeto), então uma classe usada apenas dentro daquele pacote irmão não é gerada até você adicionar o pacote à varredura. Faça isso com uma diretiva `@source` no `examples.css` (para os frames de prévia) ou no `theme.css` (para o site), escrita de forma relativa ao arquivo em que ela está — a regra padrão do Tailwind — e o Blume a leva para a folha de estilos gerada:
